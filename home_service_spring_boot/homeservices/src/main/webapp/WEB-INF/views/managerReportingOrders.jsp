    ...
    <div class="m1">
        ...
        <div class="d-flex flex-column align-items-center mt-5">
            <button type="button" class="btn btn-light btn-lg btn-block w-75 d-flex justify-content-center"
                    onclick="parent.location='/mngr/addService'">Add Service
            </button>
            <button type="button" class="btn btn-light btn-lg btn-block w-75 d-flex justify-content-center"
    ...
    <div class="m11">
        ...
        <form>
            <table class="table table-striped table-warning table-hover d-flex justify-content-center">
                ...
                <tr>
                    <td>
                        <select  style="width: 202px" size="1" name="serviceName" id="serviceName">
                            <option value="" >...Service...</option>
                            <c:forEach items="${serviceList}" var="list">
                                <option value="${list.name}">${list.name}</option>
                            </c:forEach>
                        </select>
<%--                        <input id="serviceName" name="serviceName" placeHolder="Service Name"/>--%>
                    </td>
                    <td>
                        <select  style="width: 202px" size="1" name="subServiceName" id="subServiceName">
                            <option value="" >...Sub-service...</option>
                            <c:forEach items="${SubServiceList}" var="s">
                                <option value="${s.name}">${s.name}</option>
                            </c:forEach>
                        </select>
<%--                        <input id="subServiceName" name="subServiceName" placeHolder="Sub-service Name"/>--%>
                    </td>
                    <td>
                        <select  style="width: 202px" size="1" name="orderStatus" id="orderStatus">
                            <option value="" >...Service...</option>
                            <c:forEach items="${status}" var="list">
                                <option value="${list}">${list}</option>
                            </c:forEach>
                        </select>
<%--                        <input id="orderStatus" name="orderStatus" placeHolder="Order Status"/>--%>
                    </td>
                </tr>
                <tr>
                    <td>
...
<script>
    function formToJson() {
        return JSON.stringify({
            ...
            "subServiceName": $('#subServiceName').val(),
    ...
    $(document).ready(function(){
        ...
        $("#submit").on('click', function(){
            $("#tbl1").empty()
            var row ="<tr><th >Order Id</th><th >Order Status</th><th>Customer Id</th><th>Sprcialist Id</th><th>Order Price</th><th>Register Date</th><th>Work Date</th></tr>";
            $("#tbl1 tr").remove();
            $("#tbl1").append(row)
            $.ajax({
                ...
                success : function (result) {
                    ...
                    {
                        var newrow ="";
                        newrow+="<tr><td>"+data.id+"</td><td>"+data.orderStatus+"</td><td >"+data.customerDto.id+"</td><td>"+(!data.specialistDto.id?"":data.specialistDto.id)+"</td><td>"+data.price+"</td><td>"+data.orderDate+"</td><td>"+(!data.workDate?"":data.workDate)+"</td></tr>";
                        $("#tbl1").append(newrow);
                        console.log("success")
                    });
                },
            ...
            $.ajax({
                ...
                success : function (result) {
                    ...
                    $.each(result, function (i, data)
                    {
                        console.log(i)
                        var newrow ="";
                        newrow+="<tr><td>"+data.id+"</td><td>"+data.orderStatus+"</td><td >"+data.customerDto.name+" "+data.customerDto.lastName+"</td><td>"+(!data.specialistDto?"":data.specialistDto.name)+" "+(!data.specialistDto?"":data.specialistDto.lastName)+"</td><td>"+data.price+"</td><td>"+data.orderDate+"</td><td>"+(!data.workDate?"":data.workDate)+"</td></tr>";
                        $("#tbl1").append(newrow);
                        console.log("success")
                    });
                },
